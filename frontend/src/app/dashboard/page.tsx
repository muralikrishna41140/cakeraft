...
export default function DashboardPage() {
  ...
  const handleExport = async () => {
    try {
      setIsExporting(true);
      toast.loading('Exporting revenue data to Google Sheets...', { id: 'export' });
      ...
      if (response.data.success) {
        ...
        toast.error('Export completed but no data was found to export.', { id: 'export' });
      }
    } catch (error: any) {
      console.error('Export failed:', error);
      toast.error(
        error.response?.data?.message || 'Failed to export data to Google Sheets. Please try again.',
        { id: 'export' }
      );
    } finally {
  ...
  const quickActions = [
    {
      ...
      keywords: ['order', 'billing', 'new', 'create', 'cake', 'purchase', 'buy']
    },
    {
      id: 'products',
    ...
    },
    {
      ...
      keywords: ['categories', 'organize', 'types', 'birthday', 'wedding', 'custom']
    },
    {
      id: 'analytics',
      title: 'Analytics',
      ...
      keywords: ['analytics', 'reports', 'statistics', 'revenue', 'data', 'insights']
    }
  ];
  ...
  return (
    <ProtectedRoute>
      <div className="min-h-screen bg-gradient-to-br from-white via-[#F5E8D3]/40 to-green-50/30">
        ...
        <div className={`fixed inset-y-0 left-0 z-50 w-64 sm:w-72 transform transition-transform duration-300 ease-in-out ${
          ...
          <div className="h-full bg-gradient-to-b from-white/95 to-[#F5E8D3]/20 backdrop-blur-xl border-r border-[#E9CF9C]/30 shadow-2xl shadow-green-900/5">
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-2 sm:space-x-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs sm:text-sm text-gray-600">Artisan Dashboard</p>
                  </div>
                </div>
                <button 
                  onClick={() => setIsSidebarOpen(false)}
            ...
            <nav className="p-3 sm:p-4 space-y-1 sm:space-y-2">
              {[
                { id: 'overview', label: 'Overview', icon: Layers, active: activeView === 'overview', action: () => setActiveView('overview') },
                { id: 'orders', label: 'Orders', icon: ShoppingCart, count: 5, action: () => router.push('/billing') },
                { id: 'products', label: 'Products', icon: Cake, action: () => router.push('/products') },
                { id: 'analytics', label: 'Analytics', icon: BarChart3, action: () => router.push('/analytics') },
                { id: 'categories', label: 'Categories', icon: Package, action: () => router.push('/categories/manage') },
                { id: 'export', label: 'Export Data', icon: FileSpreadsheet, active: activeView === 'export', action: () => setActiveView('export') }
              ].map((item) => (
                <button
                  key={item.id}
                  onClick={item.action}
                ...
                >
                  <div className="flex items-center space-x-3">
                    <item.icon className={`h-5 w-5 ${item.active ? 'text-white' : 'text-gray-500 group-hover:text-green-600'}`} />
                ...
                <div className="flex items-center space-x-4">
                  ...
                  <div>
                    <h2 className="text-lg sm:text-2xl font-bold text-gray-800">
                      <span className="hidden sm:inline">{getGreeting()}, Master Baker!</span>
                      <span className="sm:hidden">{getGreeting()}!</span>
                    </h2>
                    <p className="text-xs sm:text-base text-gray-600 hidden xs:block">Ready to craft some magical moments? ✨</p>
                  </div>
                </div>
              ...
              {filteredQuickActions.length > 0 ? (
                <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
                  {filteredQuickActions.map((action, index) => (
                    ...
                    >
                      <div className={`inline-flex p-3 rounded-xl bg-gradient-to-r ${action.color} mb-4 group-hover:scale-110 transition-transform duration-300 shadow-sm`}>
                ...
                <div className="text-center py-12">
                  ...
                  <h4 className="text-gray-500 font-medium mb-2">No actions found</h4>
                  <p className="text-gray-400 text-sm mb-4">
                    Try searching for "order", "products", "categories", or "analytics"
                  </p>
                  <button
                    onClick={() => setSearchQuery('')}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="lg:col-span-2">
                <div className="bg-gradient-to-br from-white to-[#F5E8D3]/30 backdrop-blur-sm rounded-2xl p-4 sm:p-6 border border-[#E9CF9C]/30 shadow-sm">
                  <div className="flex items-center justify-between mb-4 sm:mb-6">
                    <div>
                      <h3 className="text-lg sm:text-xl font-bold text-gray-800">Revenue Analytics</h3>
                      <p className="text-xs sm:text-base text-gray-600 hidden sm:block">Track your cake business performance</p>
                    </div>
              ...
              <div className="space-y-6">
                ...
                <div className="bg-gradient-to-br from-[#F5E8D3]/60 to-white rounded-2xl p-6 border border-[#E9CF9C]/50">
                  <div className="text-center">
                    ...
                    <p className="text-sm text-gray-600 mb-4">Export old revenue data to Google Sheets</p>
                    <Button
            ...
            <div className="bg-gradient-to-br from-white to-[#F5E8D3]/30 backdrop-blur-sm rounded-2xl p-6 border border-[#E9CF9C]/30 shadow-sm">
              ...
              <div className="space-y-4">
                {[
                  ...
                  <div key={index} className="flex items-center space-x-4 p-3 rounded-xl hover:bg-gradient-to-r hover:from-[#F5E8D3]/30 hover:to-green-50/30 transition-colors">
              ...
              <div className="max-w-4xl mx-auto">
                ...
                <div className="bg-gradient-to-br from-white to-[#F5E8D3]/30 backdrop-blur-sm rounded-2xl p-8 border border-[#E9CF9C]/30 shadow-sm">
                  <div className="text-center mb-8">
                    ...
                    <h2 className="text-3xl font-bold text-gray-800 mb-4">Export Data</h2>
                    <p className="text-gray-600 text-lg">Export old revenue data to Google Sheets for archival and analysis</p>
                  </div>
                  ...
                  <div className="bg-gradient-to-r from-[#F5E8D3]/60 to-green-50/60 rounded-xl p-6 border border-[#E9CF9C]/50 mb-6">
                    <div className="flex items-start space-x-4">
                      ...
                      <div>
                        <h3 className="text-lg font-semibold text-gray-800 mb-2">What happens when you export?</h3>
                        <ul className="space-y-2 text-gray-600">
                          <li className="flex items-center space-x-2">
                            <div className="w-2 h-2 bg-green-500 rounded-full"></div>
                            <span>Bills older than 30 days are exported to Google Sheets</span>
                          </li>
                          <li className="flex items-center space-x-2">
                            <div className="w-2 h-2 bg-green-500 rounded-full"></div>
                            <span>Daily revenue summaries are created for easy analysis</span>
                          </li>
                          <li className="flex items-center space-x-2">
                            <div className="w-2 h-2 bg-green-500 rounded-full"></div>
                            <span>Exported bills are safely deleted from the database</span>
                          </li>
                          <li className="flex items-center space-x-2">
                            <div className="w-2 h-2 bg-green-500 rounded-full"></div>
